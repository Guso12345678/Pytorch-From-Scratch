    ...
    def step(self, closure: None = None) -> None:  # type: ignore
        ...
        for group in self.param_groups: 
            for p in group["params"]: 
                ...
                if ("state_sum" not in param_group) and ("t" not in param_group): 
                    param_group["state_sum"] = torch.full_like(p.data, fill_value=group["initial_acumulator_value"])
                    param_group["t"] = 0 
                t = param_group["t"]
                state_sum = param_group["state_sum"] 
                t = t +1 
                lr_gorrito = group["lr"]/(1 + (t - 1)*group["lr_decay"])
                if group["weight_decay"] != 0: 
                    gradiente_p = gradiente_p + group["weight_decay"]*p.data 
                state_sum = state_sum + gradiente_p**2 
                param_group["state_sum"] = state_sum
                param_group["t"] = t 
                p.data = p.data - lr_gorrito*(gradiente_p)/(torch.sqrt(state_sum) + group["eps"])
